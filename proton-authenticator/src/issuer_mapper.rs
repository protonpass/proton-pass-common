...
mod tests {
    ...
    fn test_info_check() {
        ...
        assert!(
            aws_infos.clone().unwrap().icon_url
                == "https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=https://aws.amazon.com&size=256"
        );

        let aws_full_name_info = mapper.lookup("Amazon Web Services");
        assert!(!aws_full_name_info.clone().unwrap().domain.is_empty());
        assert!(
            aws_full_name_info.clone().unwrap().icon_url
                == "https://t0.gstatic.com/faviconV2?client=SOCIAL&type=FAVICON&fallback_opts=TYPE,SIZE,URL&url=https://aws.amazon.com&size=256"
        );
    }
    #[test]
