use crate::authenticator::parser::google::{DigitCount, OtpType};
        ...
        Ok(Self {
            totp: TOTP {
                ...
                digits: match parameters.digits.enum_value_or_default() {
                    DigitCount::DIGIT_COUNT_UNSPECIFIED => None,
                    DigitCount::DIGIT_COUNT_EIGHT => Some(8),
                    DigitCount::DIGIT_COUNT_SIX => Some(6),
                },
...
pub fn parse_google_authenticator_totp(
    ...
    let parsed =
        google::MigrationPayload::parse_from_bytes(&decoded).map_err(|_| GoogleAuthenticatorParseError::BadContent)?;

    let mut entries = vec![];

    for param in parsed.otp_parameters {
        if let Ok(v) = param.type_.enum_value() {
            if v == OtpType::OTP_TYPE_TOTP {
