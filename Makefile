...
test: ## Run the library tests
	@command_exists() { command -v cargo-nextest >/dev/null 2>&1; }; if command_exists cargo-nextest; then cargo nextest run -p proton-pass-common; else cargo test -p proton-pass-common; fi
